...
async def get_histogram_data(db: AsyncSession, broker=None):
    try:
        ...
        rtts = [r for r in result.scalars().all() if r is not None]
        if not rtts:
...
async def latency_dashboard(
    request: Request, db: AsyncSession = Depends(get_latency_db)
):
    stats = await get_latency_stats()
    recent_logs = await get_recent_logs(limit=100)

    broker_histograms = {}
    result = await db.execute(select(OrderLatency.broker).distinct())
    brokers = [b[0] for b in result.all()]
...
@limiter.limit("60/minute")
async def get_logs(
    request: Request, limit: int = 100, db: AsyncSession = Depends(get_latency_db)
):
    try:
        logs = await get_recent_logs(limit=min(limit, 1000))
