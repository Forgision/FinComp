...
from app.core.models.auth_db import get_api_key_for_tradingview, upsert_api_key
...
async def manage_api_key_get(request: Request, db = Depends(get_db), user: dict = Depends(check_session_validity_fastapi)):
    login_username = user
...
async def manage_api_key_post(request: Request, db = Depends(get_db), user: dict = Depends(check_session_validity_fastapi)):
