...
async def get_stats(current_user: dict = Depends(check_session_validity_fastapi)):
    if not current_user:
...
async def get_requests(current_user: dict = Depends(check_session_validity_fastapi)):
    if not current_user:
        return JSONResponse(content={"error": "Unauthorized"}, status_code=401)
    try:
...
async def clear_logs(current_user: dict = Depends(check_session_validity_fastapi)):
    if not current_user:
        return RedirectResponse(url="/", status_code=302)

    success, message = analyzer_service.clear_analyzer_logs()
