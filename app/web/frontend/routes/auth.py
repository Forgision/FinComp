...
@auth_router.get(
    "/reset-password", response_class=HTMLResponse, name="auth.reset_password"
)
@limiter.limit(settings.RESET_RATE_LIMIT)
async def reset_password_get(request: Request):
    return templates.TemplateResponse(
        "reset_password.html", {"request": request, "email_sent": False}
    )
    ...
    if step == "email":
        ...
        if not smtp_settings or not smtp_settings.get("smtp_server"):
            return templates.TemplateResponse(
                "reset_password.html",
                {
                    ...
                    "method_selected": False,
                    "email": email,
                    "error": "Email reset is not available. Please use TOTP authentication.",
                },
            )
            ...
            try:
                ...
                return templates.TemplateResponse(
                    "reset_password.html",
                    {
                        ...
                        "method_selected": False,
                        "email": email,
                        "error": "Failed to send reset email. Please try TOTP authentication instead.",
                    },
                )
        ...
        if not valid_token or email != request.session.get("reset_email"):
            return RedirectResponse(
                url="/auth/reset-password", status_code=status.HTTP_302_FOUND
            )
...
async def change_password_post(
    ...
    if user_obj and user_obj.check_password(old_password):
        if new_password == confirm_password:
            ...
            db.commit()
            return RedirectResponse(
                url="/auth/change", status_code=status.HTTP_302_FOUND
            )
        else:
            return RedirectResponse(
                url="/auth/change", status_code=status.HTTP_302_FOUND
...
async def configure_smtp(
    ...
    smtp_use_tls: bool = Form(...),
    smtp_from_email: str = Form(...),
    smtp_helo_hostname: str = Form(None),
):
    try:
    ...
    return RedirectResponse(
        url="/auth/change?tab=smtp", status_code=status.HTTP_302_FOUND
    )
...
async def test_smtp(
    ...
    try:
        ...
        if result["success"]:
            ...
            return JSONResponse(
                content={"success": True, "message": result["message"]}, status_code=200
            )
        else:
        ...
        return JSONResponse(
            content={"success": False, "message": error_msg}, status_code=500
        )


@auth_router.post("/debug-smtp")
async def debug_smtp(
    request: Request, user: dict = Depends(check_session_validity_fastapi)
):
    try:
...
async def logout(request: Request, db=Depends(get_db)):
    if request.session.get("logged_in"):
        ...
        inserted_id = await upsert_auth(
            db=db, username=username, password_hash="", salt="", revoke=True
        )
