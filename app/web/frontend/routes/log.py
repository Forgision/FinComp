...
async def get_filtered_logs(
    ...
    try:
        ...
        total_logs_query = select(func.count()).select_from(query.subquery())
        ...
        logs = [format_log_entry(log, ist) for log in result.scalars().all()]
...
async def view_logs(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "logs.html",
            {"request": request, "logs": [], "total_pages": 1, "current_page": 1},
        )
