...
async def telegram_index(
    ...
    try:
        ...
        stats = await get_command_stats(db, days=7)

        # Get current user's telegram link status
        telegram_user = (
            await get_telegram_user_by_username(db, current_user)
            if current_user
            else None
        )
...
async def telegram_config_get(
    ...
    config = await get_bot_config(db)
    logger.debug(
        f"Config loaded for display: broadcast_enabled={config.get('broadcast_enabled')}, bot_token={'[REDAACTED]' if config.get('bot_token') else 'absent'}"
    )
    return templates.TemplateResponse(
        "telegram/config.html", {"request": request, "config": config}
    )
...
async def telegram_config_post(
    ...
    try:
        ...
        safe_config = {
            k: "[REDACTED]" if k == "bot_token" else v for k, v in config_update.items()
        }
        logger.debug(f"Saving config: {safe_config}")
        success = await update_bot_config(db, config_update)

        if success:
            # Verify what was saved
            saved_config = await get_bot_config(db)
            logger.debug(
...
async def telegram_users(
    ...
    users = await get_all_telegram_users(db)
    stats = await get_command_stats(db, days=30)

    return templates.TemplateResponse(
        "telegram/users.html", {"request": request, "users": users, "stats": stats}
    )
...
async def telegram_analytics(
    ...
    stats_7d = await get_command_stats(db, days=7)
    stats_30d = await get_command_stats(db, days=30)
    ...
    return templates.TemplateResponse(
        "telegram/analytics.html", {"request": request, "analytics": analytics_data}
    )
...
async def stop_bot(current_user: str = Depends(check_session_validity_fastapi)):
...
async def unlink_user(
    ...
    try:
        success = await delete_telegram_user(db, telegram_id)
...
async def send_test_message(
    ...
    try:
        ...
        all_users = await get_all_telegram_users(db)
        ...
        if telegram_bot_service.bot_loop and telegram_bot_service.is_running:
            success = await asyncio.to_thread(
                telegram_bot_service.send_notification,
...
async def send_message(
    ...
    try:
        ...
        if not telegram_id or not message:
            raise HTTPException(
                status_code=400, detail="Missing telegram_id or message"
            )
        ...
        if len(message) > 4096:  # Telegram's max message length
            raise HTTPException(
                status_code=400, detail="Message too long (max 4096 characters)"
            )
