...
async def security_dashboard(
    ...
    try:
        ...
        # Get all banned IPs
        banned_ips = await IPBan.get_all_bans(logs_db)

        # Get suspicious IPs (1+ 404 errors to show all tracking)
        suspicious_ips = await Error404Tracker.get_suspicious_ips(logs_db, min_errors=1)

        # Get suspicious API users (1+ invalid API key attempts to show all)
        suspicious_api_users = await InvalidAPIKeyTracker.get_suspicious_api_users(
            logs_db, min_attempts=1
        )
        ...
        banned_data = [
            {
                ...
                "banned_at": (
                    ban.banned_at.strftime("%d-%m-%Y %I:%M:%S %p")
                    if ban.banned_at
                    else "Unknown"
                ),
                "expires_at": (
                    ban.expires_at.strftime("%d-%m-%Y %I:%M:%S %p")
                    if ban.expires_at
                    else "Permanent"
                ),
        ...
        suspicious_data = [
            {
                ...
                "first_error_at": (
                    tracker.first_error_at.strftime("%d-%m-%Y %I:%M:%S %p")
                    if tracker.first_error_at
                    else "Unknown"
                ),
                "last_error_at": (
                    tracker.last_error_at.strftime("%d-%m-%Y %I:%M:%S %p")
        ...
        api_abuse_data = [
            {
                ...
                "first_attempt_at": (
                    tracker.first_attempt_at.strftime("%d-%m-%Y %I:%M:%S %p")
                    if tracker.first_attempt_at
                    else "Unknown"
                ),
                "last_attempt_at": (
                    tracker.last_attempt_at.strftime("%d-%m-%Y %I:%M:%S %p")
        ...
        return templates.TemplateResponse(
            "security/dashboard.html",
            {
                ...
                "suspicious_ips": [],
                "api_abuse_ips": [],
                "security_settings": await get_security_settings(
                    db
                ),  # Still try to get settings if possible
...
async def ban_ip(
    ...
    if not ip_address:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="IP address is required"
        )

    # Prevent banning localhost
    if ip_address in ["127.0.0.1", "::1", "localhost"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot ban localhost"
        )

    try:
        success = await IPBan.ban_ip(
...
async def unban_ip(
    ...
    try:
        success = await IPBan.unban_ip(logs_db, ip_address)
        ...
        if success:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="IP not found in ban list"
            )
    except Exception as e:
...
async def ban_host(
    ...
    if not host:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Host is required"
        )
    ...
    if ip_pattern.match(host):
        ...
        try:
            ...
            if success:
                return {"success": True, "message": f"Banned IP: {host}"}
            else:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Failed to ban IP: {host}",
                )
        except Exception as e:
    ...
    matching_logs = (
        await logs_db.query(TrafficLog)
        .filter(TrafficLog.host.like(f"%{host}%"))
...
async def clear_404_tracker(
    ...
    try:
        tracker = (
            await logs_db.query(Error404Tracker)
            .filter_by(ip_address=ip_address)
            .first()
        )
        if tracker:
            await logs_db.delete(tracker)
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="No tracker found for this IP",
            )
    except Exception as e:
        logger.error(f"Error clearing 404 tracker: {e}")
        await logs_db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
        )
...
async def security_stats(
    ...
    try:
        # Count banned IPs
        total_bans = await logs_db.query(IPBan).count()
        permanent_bans = await logs_db.query(IPBan).filter_by(is_permanent=True).count()
        temp_bans = total_bans - permanent_bans

        # Count suspicious IPs
        suspicious_count = (
            await logs_db.query(Error404Tracker)
            .filter(Error404Tracker.error_count >= 5)
            .count()
        )
        ...
        near_threshold = (
            await logs_db.query(Error404Tracker)
            .filter(Error404Tracker.error_count >= 15, Error404Tracker.error_count < 20)
            .count()
        )
...
async def update_security_settings(
    ...
    if not (1 <= threshold_404 <= 1000):
        raise HTTPException(
            ...
            detail="404 threshold must be between 1 and 1000",
        )
    if not (1 <= ban_duration_404 <= 8760):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Ban duration must be between 1 hour and 1 year",
        )
    if not (1 <= threshold_api <= 100):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="API threshold must be between 1 and 100",
        )
    ...
    if not (1 <= repeat_offender_limit <= 10):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Repeat offender limit must be between 1 and 10",
        )

    try:
        await set_security_settings(
