...
async def getKotakOTP(userid: str, access_token: str):
    ...
    async with await get_httpx_client() as client:
        payload = json.dumps(
            {"userId": userid, "sendEmail": True, "isWhitelisted": True}
        )
        headers = {
            "accept": "*/*",
...
# TODO: The original Flask app used `limiter.limiter`. This needs to be replaced with FastAPI-compatible rate limiting.
...
@broker_router.get(
    ...
    dependencies=[
        Depends(RateLimiter(times=settings.LOGIN_RATE_LIMIT_MIN, seconds=60)),
        Depends(RateLimiter(times=settings.LOGIN_RATE_LIMIT_HOUR, seconds=3600)),
    ],
)
...
async def broker_callback(
    broker: str, request: Request, db: AsyncSession = Depends(get_db)
):
    ...
    if broker == "compositedge" and "user" not in request.session:
        ...
        logger.info(
            "Compositedge callback without session - will establish session after auth"
        )
    else:
    ...
    if broker == "fivepaisa":
        if request.method == "GET":
            return JSONResponse(
                content={"message": "GET not supported, please POST credentials."},
            ...
            auth_token, feed_token, error_message = await auth_function(
                    ...
                    if data_dict.get("stat") == "Ok" and data_dict.get("encKey"):
                        ...
                        if auth_token:
                            return await handle_auth_success(
                                request, db, auth_token, request.session["user"], broker
                            )
                        else:
                            return JSONResponse(
                        ...
                        error_msg = data_dict.get(
                            "emsg", "Failed to get encryption key"
                        )
                        return JSONResponse(
                            content={
                                "status": "error",
                                "message": f"Failed to get encryption key: {error_msg}",
                            },
        ...
        try:
            if request.method == "POST":
                ...
                if content_type and "application/x-www-form-urlencoded" in content_type:
                    form = await request.form()
                    raw_data_form = form.get(
                        "session"
                    )  # Assuming 'session' is the form field name
                    if raw_data_form:
                        ...
                        raw_data = (await request.body()).decode(
                            "utf-8"
                        )  # Fallback if 'session' not in form
            ...
            if not raw_data:
                return JSONResponse(
                    content={"error": "No session data received"},
            ...
            if not isinstance(session_json, dict):
                logger.error(
                    f"Parsed session_json is not a dictionary: {type(session_json)}"
                )
                return JSONResponse(
                    content={"error": "Invalid session data format after JSON parsing"},
                    status_code=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not access_token:
                return JSONResponse(
                    content={"error": "Access token not found in session data"},
                    status_code=status.HTTP_400_BAD_REQUEST,
                )

            # auth_function is defined earlier in the outer scope, so it's accessible here.
            auth_token, feed_token, user_id, error_message = await auth_function(
                access_token
            )

        except json.JSONDecodeError as e:
            logger.error(
                f"JSONDecodeError in compositedge callback: {e}, raw_data: {raw_data}"
            )
            return JSONResponse(
                content={
        ...
        if request.method == "GET":
            ...
            auth_token, error_message = await auth_function(
        ...
        if auth_token:
            ...
            if not is_valid:
                logger.error(
                    f"Dhan authentication validation failed: {validation_error}"
                )
                return JSONResponse(
                    content={
                        "status": "error",
                        "message": f"Authentication validation failed: {validation_error}",
                    },
                    status_code=status.HTTP_401_UNAUTHORIZED,
                )
        ...
        if request.method == "GET":
            ...
            auth_token, error_message = await auth_function(
                otp, token, sid, userid, access_token, hsServerId
            )
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if step1_response and "otp_token" in step1_response:
                    ...
                    otp_message = step1_response.get(
            ...
            if action == "resend":
                ...
                try:
                    step1_response = await login_step1(api_token, api_secret)
                    if step1_response and "otp_token" in step1_response:
                        request.session["definedge_otp_token"] = step1_response[
                            "otp_token"
                        ]
                        otp_message = "OTP has been resent successfully"
                        logger.info("Definedge OTP resent successfully")
                        return JSONResponse(
                            content={"status": "success", "message": otp_message}
                        )
                    else:
                        return JSONResponse(
                            content={
                ...
                try:
                    (
                        ...
                        user_id,
                        error_message,
                    ) = await authenticate_broker(otp_token, otp_code, api_secret)
        ...
        if (
            ...
            if broker == "compositedge" and "user" not in request.session:
                admin_user = await user_service.get_user_by_username(
                    db, username="admin"
                )  # Corrected function call and added username parameter
                if admin_user:
                    username = admin_user.username
                    request.session["user"] = username
                    logger.info(
                        f"Compositedge callback: Set session user to {username}"
                    )
                else:
                    logger.error(
                        "No admin user found in database for Compositedge callback"
                    )
                    return await handle_auth_failure(
                        request,
                        "No user account found. Please login first.",
...
async def broker_loginflow(broker: str, request: Request):
        ...
        if request.method == "POST":
            ...
            async with await get_httpx_client() as client:
                ...
                auth_string = base64.b64encode(
                    f"{settings.BROKER_API_KEY}:{api_secret}".encode()
                ).decode("utf-8")
    ...
    return HTMLResponse(
        content="Unsupported broker or method for loginflow",
        status_code=status.HTTP_400_BAD_REQUEST,
    )
