...
async def tradingview_json_get(
    request: Request, user: str = Depends(check_session_validity_fastapi)
):
    return templates.TemplateResponse(
        "tradingview.html", {"request": request, "host": host}
    )
...
async def tradingview_json_post(
    ...
    ),  # Renamed to avoid conflict with 'user'
):
    try:
        ...
        logger.info(
            f"Processing TradingView request - Symbol: {symbol_input}, Exchange: {exchange}, Product: {product}"
        )

        # Get actual API key for TradingView
        api_key = await get_api_key_for_tradingview(db, user_data.get("user"))
        ...
        symbols = await enhanced_search_symbols(db, symbol_input, exchange)
        if not symbols:
