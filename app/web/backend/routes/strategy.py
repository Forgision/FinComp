...
def process_orders():
    ...
    while True:
        try:
            ...
            now = (
                time_module.time()
            )  # Changed to time_module.time() to avoid conflict with datetime.time
...
def validate_strategy_times(
    start_time_str: str, end_time_str: str, squareoff_time_str: str
):
...
def schedule_squareoff(db: AsyncSession, strategy_id):
...
def squareoff_positions(strategy_id: int, db: AsyncSession):
...
async def index(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "strategy/index.html",
            {"request": request, "strategies": strategies, "user_id": user_id},
        )
...
async def new_strategy_get(
    ...
    return templates.TemplateResponse(
        "strategy/new_strategy.html", {"request": request, "user_id": user_id}
    )
...
async def new_strategy_post(
    ...
    try:
        ...
        if not is_valid_name:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=name_error
            )

        # Validate times for intraday strategy
        is_intraday = strategy_type == "intraday"
        if is_intraday:
            is_valid_time, time_error = validate_strategy_times(
                start_time, end_time, squareoff_time
            )
            if not is_valid_time:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=time_error
                )
        else:
        ...
        if strategy:
            ...
            return RedirectResponse(
                url=strategy_router.url_path_for(
                    "configure_symbols", strategy_id=strategy.id
                ),
...
async def toggle_strategy_route(
    ...
    try:
        ...
        if strategy:
            ...
            return RedirectResponse(
                url=strategy_router.url_path_for(
                    "view_strategy", strategy_id=strategy_id
                ),
                status_code=status.HTTP_302_FOUND,
            )
        else:
...
@strategy_router.post(
    "/{strategy_id}/symbol/{mapping_id}/delete", response_class=JSONResponse
)
async def delete_symbol(
    ...
    try:
        if delete_symbol_mapping(db, mapping_id):
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Symbol mapping not found"
            )
    except HTTPException:
        raise
    except Exception as e:
...
async def webhook(
    ...
    try:
        ...
        if not strategy.is_active:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Strategy is inactive"
            )
        ...
        if strategy.is_intraday:
            ...
            if not is_exit_order:
                if strategy.start_time and current_time < strategy.start_time:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Entry orders not allowed before start time",
                    )

                if strategy.end_time and current_time > strategy.end_time:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Entry orders not allowed after end time",
                    )
            ...
            # For exit orders, check if within exit time window (up to square off time)
                ...
                if strategy.start_time and current_time < strategy.start_time:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Exit orders not allowed before start time",
                    )

                if strategy.squareoff_time and current_time > strategy.squareoff_time:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Exit orders not allowed after square off time",
                    )
        ...
        if strategy.trading_mode == "LONG":
            if action not in ["BUY", "SELL"]:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Invalid action for LONG mode. Use BUY to enter, SELL to exit",
                )
            ...
            if action not in ["BUY", "SELL"]:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Invalid action for SHORT mode. Use SELL to enter, BUY to exit",
                )
            ...
            if action == "BUY" and position_size < 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="For BUY orders in BOTH mode, position_size must be >= 0",
                )
            if action == "SELL" and position_size > 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="For SELL orders in BOTH mode, position_size must be <= 0",
                )
        ...
        mapping = next(
            (
                m
                for m in get_symbol_mappings(db, strategy.id)
                if m.symbol == data["symbol"]
            ),
            None,
        )
        if not mapping:
        ...
        if not api_key:
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="No API key found"
            )
        ...
        if strategy.trading_mode == "BOTH":
            ...
            if use_smart_order:
                ...
                quantity = (
                    abs(position_size) if position_size != 0 else mapping.quantity
                )
