...
async def websocket_dashboard(
    request: Request, username: str = Depends(check_session_validity_fastapi)
):
...
async def api_websocket_subscriptions(
    username: str = Depends(get_username_from_session_fastapi),
):
...
async def api_websocket_subscribe(
    request: Request, username: str = Depends(get_username_from_session_fastapi)
):
    ...
    success, result, status_code = await subscribe_to_symbols(
...
async def api_websocket_unsubscribe(
    request: Request, username: str = Depends(get_username_from_session_fastapi)
):
    ...
    success, result, status_code = await unsubscribe_from_symbols(
        username, broker, symbols, mode
    )
    return JSONResponse(content=result, status_code=status_code)
...
async def api_websocket_unsubscribe_all(
    request: Request, username: str = Depends(get_username_from_session_fastapi)
):
...
async def api_get_websocket_apikey(
    username: str = Depends(get_username_from_session_fastapi),
):
    ...
    if not username:
        raise HTTPException(
            ...
            detail="Session not found - please refresh page",
        )
    ...
    if not api_key:
        raise HTTPException(
            ...
            detail="No API key found. Please generate an API key first.",
        )

    return JSONResponse(
        content={"status": "success", "api_key": api_key}, status_code=200
    )


@websocket_router.get("/api/websocket/config")
async def api_get_websocket_config(
    request: Request, username: str = Depends(get_username_from_session_fastapi)
):
...
async def handle_disconnect(sid: str):
    ...
    if username:
        ...
        logger.info(
            f"User {username} disconnected from market data stream (SID: {sid})"
        )
...
async def handle_subscribe(sid: str, data: Dict[str, Any]):
    ...
    if not username:
        await sio.emit(
            "error", {"message": "Not authenticated"}, room=sid, namespace="/market"
        )
        return
