...
async def sandbox_config(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        organized_configs = {
            ...
            "square_off": {
                ...
                "configs": {
                    "nse_bse_square_off_time": configs.get(
                        "nse_bse_square_off_time", {}
                    ),
                    "cds_bcd_square_off_time": configs.get(
        ...
        logger.error(
            f"Error rendering sandbox config: {str(e)}\n{traceback.format_exc()}"
        )
...
async def update_config(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not config_key or config_value is None:
            raise HTTPException(
                status_code=400, detail="Missing config_key or config_value"
            )
        ...
        if success:
            ...
            if config_key.endswith("square_off_time") or config_key in [
                "reset_day",
                "reset_time",
            ]:
                try:
        ...
        logger.error(
            f"Error updating sandbox config: {str(e)}\n{traceback.format_exc()}"
        )
        raise HTTPException(
...
async def reset_config(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        try:
            # Delete all orders
            deleted_orders = (
                await db.execute(delete(SandboxOrders).filter_by(user_id=user_id))
            ).rowcount
            logger.info(f"Deleted {deleted_orders} sandbox orders for user {user_id}")

            # Delete all trades
            deleted_trades = (
                await db.execute(delete(SandboxTrades).filter_by(user_id=user_id))
            ).rowcount
            logger.info(f"Deleted {deleted_trades} sandbox trades for user {user_id}")

            # Delete all positions
            deleted_positions = (
                await db.execute(delete(SandboxPositions).filter_by(user_id=user_id))
            ).rowcount
            logger.info(
                f"Deleted {deleted_positions} sandbox positions for user {user_id}"
            )

            # Delete all holdings
            deleted_holdings = (
                await db.execute(delete(SandboxHoldings).filter_by(user_id=user_id))
            ).rowcount
            logger.info(
                f"Deleted {deleted_holdings} sandbox holdings for user {user_id}"
            )
            ...
            fund = (
                (await db.execute(select(SandboxFunds).filter_by(user_id=user_id)))
                .scalars()
                .first()
            )
            ...
            await db.rollback()
            logger.error(
                f"Error clearing sandbox data: {str(e)}\n{traceback.format_exc()}"
            )
            raise HTTPException(
                status_code=500, detail=f"Error clearing sandbox data: {str(e)}"
            )
        ...
        logger.error(
            f"Error resetting sandbox config: {str(e)}\n{traceback.format_exc()}"
        )
        raise HTTPException(
...
async def reload_squareoff(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(
            f"Error reloading square-off schedule: {str(e)}\n{traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=500, detail=f"Error reloading square-off schedule: {str(e)}"
        )


@sandbox_router.get("/squareoff-status")
async def squareoff_status(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(
            f"Error getting square-off status: {str(e)}\n{traceback.format_exc()}"
        )
        raise HTTPException(
            status_code=500, detail=f"Error getting square-off status: {str(e)}"
        )
