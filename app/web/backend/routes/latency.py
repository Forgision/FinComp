...
from app.core.schemas.latency_db import OrderLatency, get_recent_logs, get_latency_stats
...
async def get_histogram_data(db: AsyncSession, broker: str = None):
    ...
    try:
        query = await db.execute(select(OrderLatency.rtt_ms))
        if broker:
            query = query.filter(OrderLatency.broker == broker)

        # Get all RTT values
        rtts = [r[0] for r in query.scalars().all()]
