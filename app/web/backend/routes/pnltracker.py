...
async def pnltracker(request: Request, user_data: str = Depends(check_session_validity_fastapi)):
    """Render the PnL tracker page."""
    return templates.TemplateResponse("pnltracker.html", {"request": request, "user": user_data})
...
async def get_pnl_data(
    ...
    try:
        ...
        success, tradebook_response, status_code = await get_tradebook(api_key=api_key) # Assuming get_tradebook is async
        ...
        mapping_funcs = dynamic_import(broker, 'mapping.transform_data', [
        ...
        try:
            ...
            positions_data = await get_positions(auth_token) # Assuming get_positions is async
        ...
        for symbol_key, trades_list in symbol_trades.items():
            ...
            try:
                success, hist_response, _ = await get_history( # Assuming get_history is async
                    symbol=symbol,
                    exchange=exchange,
                    interval='1m',
