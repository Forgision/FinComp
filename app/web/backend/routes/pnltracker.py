            ...
            try:
                ...
                try:
                    ...
                    if df["datetime"].dt.tz is None:
                        df["datetime"] = (
                            df["datetime"].dt.tz_localize("UTC").dt.tz_convert(ist)
                        )
                    else:
                        df["datetime"] = df["datetime"].dt.tz_convert(ist)
...
@pnltracker_router.get("/", response_class=HTMLResponse)
async def pnltracker(
    request: Request, user_data: str = Depends(check_session_validity_fastapi)
):
    """Render the PnL tracker page."""
    return templates.TemplateResponse(
        "pnltracker.html", {"request": request, "user": user_data}
    )


@pnltracker_router.post("/api/pnl", response_class=JSONResponse)
async def get_pnl_data(
    request: Request,
    db: AsyncSession = Depends(get_db),
    user_data: str = Depends(check_session_validity_fastapi),
):
    ...
    try:
        ...
        if not api_key:
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="API key not configured. Please generate an API key in /apikey",
            )
        ...
        success, tradebook_response, status_code = await get_tradebook(
            api_key=api_key
        )  # Assuming get_tradebook is async

        if not success:
            logger.error(f"Error fetching tradebook: {tradebook_response}")
            raise HTTPException(
                status_code=status_code,
                detail=tradebook_response.get("message", "Error fetching tradebook"),
            )
        ...
        try:
            ...
            positions_data = await get_positions(
                auth_token
            )  # Assuming get_positions is async

            if positions_data and (
                "status" not in positions_data or positions_data["status"] != "error"
            ):
                ...
                logger.info(
                    f"Number of positions: {len(positions_data) if positions_data else 0}"
                )
                for pos in positions_data:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error converting position values to float for {key}"
                        )
                        qty = 0
                        avg_price = 0
                        ltp = 0
                    ...
                    logger.info(
                        f"Position {key}: qty={qty}, avg={avg_price}, ltp={ltp}, pnl={pnl}"
                    )
        except Exception as e:
        ...
        for trade in trades:
            ...
            if trade_timestamp:
                try:
                    ...
                    if (
                        isinstance(trade_timestamp, str)
                        and ":" in trade_timestamp
                        and len(trade_timestamp.split(":")[0]) <= 2
                    ):
                    ...
                    logger.warning(
                        f"Could not parse trade timestamp {trade_timestamp}: {e}"
                    )
                ...
                if fill_time_str:
                    try:
                        ...
                        if ":" in str(fill_time_str):
                            ...
                            if (
                                first_trade_time is None
                                or trade_time < first_trade_time
                            ):
                                first_trade_time = trade_time
                                logger.info(
                        ...
                        logger.warning(
                            f"Could not parse fill_time {fill_time_str}: {e}"
                        )

        # Log the first trade time
        if first_trade_time:
            logger.info(
                f"First trade time: {first_trade_time.strftime('%Y-%m-%d %H:%M:%S %Z')}"
            )
        else:
            logger.warning(
                "Could not determine first trade time, using market open time"
            )
            ist = pytz.timezone("Asia/Kolkata")
            first_trade_time = datetime.now(ist).replace(
        ...
        for symbol_key, trades_list in symbol_trades.items():
            ...
            # Sort trades by time
            trades_list.sort(
                key=lambda x: x.get("parsed_time")
                or datetime.min.replace(tzinfo=pytz.UTC)
            )
            ...
            for trade in trades_list:
                try:
                    ...
                    if qty <= 0:
                        logger.warning(
                            f"Skipping trade with zero/negative quantity: {trade}"
                        )
                        continue
                    ...
                    if not df_hist.empty:
                        ...
                        if df_hist is not None:
                            ...
                            for window in position_windows_sorted:
                                ...
                                if not mask.any():
                                    logger.warning(
                                        f"No data points found for position window from {start} to {end}"
                                    )
                                    continue
                                ...
                                if window["action"] == "BUY":
                                    position_pnl = (
                                        df_hist.loc[mask, f"{symbol}_price"]
                                        - window["price"]
                                    ) * window["qty"]
                                    ...
                                    if window["end_time"] and window.get("exit_price"):
                                        ...
                                        logger.info(
                                            f"Closed BUY position: entry={window['price']}, exit={window['exit_price']}, "
                                    ...
                                    if window["end_time"] and window.get("exit_price"):
                                        ...
                                        cumulative_realized_pnl += realized
                                        logger.info(
                                            f"Closed SELL position: entry={window['price']}, exit={window['exit_price']}, "
                                            f"qty={window['qty']}, realized PnL={realized}"
                                        )
                            ...
                            logger.info(
                                f"Added PnL for {symbol}: {len(df_hist)} data points"
                            )
                        else:
        ...
        if portfolio_pnl is None and current_positions:
            ...
            for pos_key, pos_data in current_positions.items():
                ...
                try:
                    ...
                    if success and "data" in hist_response:
                        ...
                        logger.warning(
                            f"Could not get historical data for position {symbol}"
                        )
            ...
            if portfolio_pnl is None:
                ...
                time_range = pd.date_range(
                    start=start_time, end=end_time, freq="1min", tz=ist
                )
            ...
            if first_trade_time and trades:
                ist = pytz.timezone("Asia/Kolkata")
                market_open = first_trade_time.replace(
                    hour=9, minute=15, second=0, microsecond=0
                )
        ...
        # Calculate metrics
        latest_mtm = (
            portfolio_pnl["Total_PnL"].iloc[-1] if not portfolio_pnl.empty else 0
        )
        ...
        if not portfolio_pnl.empty:
            for idx, row in portfolio_pnl.iterrows():
                try:
                    ...
                    pnl_series.append(
                        {"time": timestamp_ms, "value": round(float(pnl_value), 2)}
                    )
