...
master_contract_status_router = APIRouter(
    prefix="/api/master-contract", tags=["Master Contract Status"]
)
...
async def get_master_contract_status(
    ...
    try:
        ...
        status_data = await get_status(
            db, broker
        )  # Assuming get_status is now async and takes db
...
async def check_master_contract_ready(
    ...
    try:
        ...
        is_ready = await check_if_ready(
            db, broker
        )  # Assuming check_if_ready is now async and takes db
        return JSONResponse(
            content={
                "ready": is_ready,
...
async def get_cache_status(
    ...
    try:
        cache_info = await get_cache_stats(
            db
        )  # Assuming get_cache_stats is now async and takes db
...
async def get_cache_health_fastapi(
    ...
    try:
        health_info = await get_cache_health(
            db
        )  # Assuming get_cache_health is now async and takes db
...
async def reload_cache_fastapi(
    ...
    try:
        ...
        success = await load_symbols_to_cache(
            db, broker
        )  # Assuming load_symbols_to_cache is now async and takes db
...
async def clear_cache_fastapi(
    ...
    try:
        ...
        return JSONResponse(
            content={"status": "success", "message": "Cache cleared successfully"},
            status_code=status.HTTP_200_OK,
        )
