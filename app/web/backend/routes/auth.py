...
async def login_get(
    ...
    if await user_service.get_total_users_count(db) == 0:
        return RedirectResponse(
            url=auth_router.url_path_for("setup"), status_code=status.HTTP_302_FOUND
        )
...
async def logout(request: Request, db: AsyncSession = Depends(get_db)):
    if "user" in request.session:
        ...
        inserted_id = await upsert_auth(
            db=db, name=username, auth_token="", broker="", revoke=True
        )
        if inserted_id:
...
async def change_password_get(request: Request, db: AsyncSession = Depends(get_db)):
...
async def change_password_post(
    ...
    return RedirectResponse(
        url=request.url_for("auth.change"), status_code=status.HTTP_303_SEE_OTHER
    )
...
@auth_router.get(
    "/reset-password", name="auth.reset_password", response_class=HTMLResponse
)
...
async def reset_password_post(
    ...
    if step == "email":
        ...
        return JSONResponse(
            content={"email_sent": True, "method_selected": False, "email": email}
        )
        ...
        if user:
            try:
                ...
                send_password_reset_email(email, reset_link, db, user.username)
                logger.info(f"Password reset email sent to {email}")
            except Exception as e:
                logger.error(f"Failed to send password reset email to {email}: {e}")
                flash(
                    request,
                    "Failed to send reset email. Please try TOTP authentication instead.",
                    "error",
                )
                return JSONResponse(
                    content={
        ...
        valid_token = token == request.session.get(
            "reset_token"
        ) or token == request.session.get("email_reset_token")
        if not valid_token or email != request.session.get("reset_email"):
            flash(request, "Invalid or expired reset token.", "error")
            return RedirectResponse(
                url=request.url_for("reset_password"), status_code=status.HTTP_302_FOUND
            )
...
@auth_router.post(
    "/smtp-config", name="auth.configure_smtp", status_code=status.HTTP_200_OK
)
async def configure_smtp(
    request: Request, smtp_config: SMTPConfig, db: AsyncSession = Depends(get_db)
):
    """
    ...
    if "user" not in request.session:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="You must be logged in"
        )

    try:
        await set_smtp_settings(
            db,
            ...
            smtp_from_email=(
                str(smtp_config.smtp_from_email)
                if smtp_config.smtp_from_email
                else None
            ),
        ...
        raise HTTPException(
            ...
            detail=f"Error updating SMTP settings: {str(e)}",
        )


@auth_router.post("/test-smtp", name="auth.test_smtp", status_code=status.HTTP_200_OK)
async def test_smtp(
    request: Request, smtp_test: SMTPTest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        result = await send_test_email(
            smtp_test.test_email, sender_name=request.session["user"], db=db
        )
        if result["success"]:
            logger.info(
                f"Test email sent successfully by user: {request.session['user']} to {smtp_test.test_email}"
            )
            return result
        else:
            logger.warning(
                f"Test email failed for user: {request.session['user']} - {result['message']}"
            )
            raise HTTPException(
...
async def debug_smtp(request: Request, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        result = await debug_smtp_connection(db=db)
        return result
    except Exception as e:
