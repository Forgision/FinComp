...
def save_env_variables(
    strategy_id: str,
    regular_vars: Dict[str, str],
    secure_vars: Optional[Dict[str, str]] = None,
):
...
async def get_active_broker(db: AsyncSession):
...
async def check_master_contract_ready(
    ...
    try:
        ...
        if not broker:
            ...
            if skip_on_startup:
                logger.info(
                    "No broker found during startup - skipping master contract check"
                )
    ...
    if dt:
        if isinstance(dt, str):
            try:
                ...
                logger.warning(f"Could not parse datetime string '{dt}': {e}")
                return dt
        if not dt.tzinfo:
            dt = IST.localize(dt)
        else:
    ...
    with PROCESS_LOCK:  # Thread-safe operation
        ...
        if not IS_WINDOWS:
            ...
            if not os.access(file_path, os.R_OK):
                logger.error(
                    f"Strategy file {file_path} is not readable. Check file permissions."
                )
                return (
                    False,
                    f"Strategy file is not readable. Run: chmod +r {file_path}",
                )
        ...
        try:
            ...
            log_file = (
                LOGS_DIR / f"{strategy_id}_{ist_now.strftime('%Y%m%d_%H%M%S')}_IST.log"
            )
            ...
            if env_vars:
                ...
                logger.info(
                    f"Loaded {len(env_vars)} environment variables for strategy {strategy_id}"
                )
        ...
        try:
            ...
            if "log_handle" in strategy_info and strategy_info["log_handle"]:
                try:
                    ...
                    logger.error(
                        f"Error closing log handle for strategy {strategy_id}: {e}"
                    )
                    pass
            ...
            logger.info(
                f"Stopped strategy {strategy_id} at {ist_now.strftime('%H:%M:%S IST')}"
            )
...
def schedule_strategy(
    ...
    logger.info(
        f"Scheduled strategy {strategy_id}: {start_time} - {stop_time} IST on {days}"
    )
...
async def new_strategy_get(request: Request):
    ...
    return templates.TemplateResponse(
        "python_strategy/new.html", {"request": request, "flash_messages": []}
    )


@python_strategy_router.post(
    "/new", dependencies=[Depends(check_session_validity_fastapi)]
)
async def new_strategy_post(
    ...
    ist_now = get_ist_time()
    strategy_id = (
        Path(strategy_file.filename).stem + "_" + ist_now.strftime("%Y%m%d%H%M%S")
    )
    ...
    try:
        ...
        return templates.TemplateResponse(
            "python_strategy/new.html",
            {
                "request": request,
                "flash_messages": [
                    {"message": f"Failed to save file: {e}", "category": "error"}
                ],
            },
        )
...
@python_strategy_router.post(
    "/stop/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
...
@python_strategy_router.post(
    "/unschedule/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
...
@python_strategy_router.post(
    "/delete/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
async def delete_strategy(strategy_id: str):
    ...
    with PROCESS_LOCK:  # Thread-safe operation
        ...
        if strategy_id in RUNNING_STRATEGIES or (
            strategy_id in STRATEGY_CONFIGS
            and STRATEGY_CONFIGS[strategy_id].get("is_running")
        ):
...
@python_strategy_router.post(
    "/logs/{strategy_id}/clear", dependencies=[Depends(check_session_validity_fastapi)]
)
async def clear_logs(strategy_id: str):
    ...
    try:
        ...
        if not log_files:
            return JSONResponse(
                {"success": False, "message": "No log files found to clear"}
            )
                ...
                if strategy_id in RUNNING_STRATEGIES:
                    ...
                    if active_log_file and Path(active_log_file).name == log_file.name:
                        ...
                        with open(log_file, "w", encoding="utf-8") as f:
                            f.write(
                                f"=== Log cleared at {get_ist_time().strftime('%Y-%m-%d %H:%M:%S IST')} ===\n"
                            )
                        logger.info(
                            f"Truncated active log file for running strategy {strategy_id}"
                        )
                    else:
...
@python_strategy_router.post(
    "/clear-error/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
async def clear_error_state(strategy_id: str):
    ...
    if config.get("is_running"):
        raise HTTPException(
            status_code=400, detail="Cannot clear error state while strategy is running"
        )
...
@python_strategy_router.get(
    "/export/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
...
@python_strategy_router.post(
    "/save/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
async def save_strategy(strategy_id: str, data: Dict[str, Any]):
    ...
    if config.get("is_running", False):
        raise HTTPException(
            status_code=400,
            detail="Cannot edit running strategy. Please stop it first.",
        )
...
@python_strategy_router.get(
    "/env/{strategy_id}", dependencies=[Depends(check_session_validity_fastapi)]
)
...
async def manage_env_variables_post(strategy_id: str, data: Dict[str, Any]):
    ...
    try:
        ...
        # Save variables
        save_env_variables(
            strategy_id, regular_vars, secure_vars if secure_vars else None
        )
...
async def restore_strategy_states(db: AsyncSession, request: Request):
    ...
    if "No broker" in contract_message:
        logger.info(
            "No active broker found during startup - delaying strategy restoration"
        )
        ...
        for strategy_id, config in STRATEGY_CONFIGS.items():
            if config.get("is_running"):
                ...
                config["error_message"] = (
                    "Waiting for master contracts to be downloaded"
                )
            ...
            try:
                ...
                if psutil.pid_exists(pid):
                    ...
                    if strategy_file and strategy_file in cmdline:
                        ...
                        logger.info(
                            f"Restored running strategy {strategy_id} (PID: {pid})"
                        )
                        restored_count += 1
