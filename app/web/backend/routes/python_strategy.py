...
from app.db.models.auth_db import Auth as DBAuth
from app.db.models.master_contract_status_db import (
        ...
        try:
            ...
            if not IS_WINDOWS:
                try:
                    ...
                    logger.warning(f"Could not set permissions for log directory {log_file.parent}: {e}")
                    pass
            ...
            if strategy_id in STRATEGY_CONFIGS:
                ...
                if pid and check_process_status(pid):
                    try:
                        ...
                        logger.error(f"Error stopping orphaned process {pid} for strategy {strategy_id}: {e}")
                        pass
        ...
        try:
            ...
            if 'log_handle' in strategy_info and strategy_info['log_handle']:
                try:
                    ...
                    logger.error(f"Error closing log handle for strategy {strategy_id}: {e}")
                    pass
        ...
        for strategy_id, info in list(RUNNING_STRATEGIES.items()):
            ...
            if isinstance(process, subprocess.Popen):
                ...
                try:
                    ...
                    logger.error(f"Error checking PID {pid} for strategy {strategy_id}: {e}")
                    is_dead = True
            ...
            if is_dead:
                ...
                if 'log_handle' in info and info['log_handle']:
                    try:
                        ...
                        logger.error(f"Error closing log handle for dead strategy {strategy_id}: {e}")
                        pass
