...
async def traffic_dashboard(
    ...
    stats = await TrafficLog.get_stats(db)
    recent_logs = await TrafficLog.get_recent_logs(db, limit=100)
...
async def get_logs(
    ...
    try:
        ...
        logs = await TrafficLog.get_recent_logs(db, limit=limit)
        return JSONResponse(
            content=[
                {
                    "timestamp": format_ist_time(log.timestamp),
...
async def get_stats(
    ...
    try:
        # Get overall stats
        # all_logs_result = await db.execute(select(TrafficLog))
        overall_stats = {
            "total_requests": (
                await db.execute(select(func.count()).select_from(TrafficLog))
            ).scalar_one(),
            "error_requests": (
                await db.execute(
                    select(func.count()).filter(TrafficLog.status_code >= 400)
                )
            ).scalar_one(),
            "avg_duration": round(
                float(
                    (
                        await db.execute(select(func.avg(TrafficLog.duration_ms)))
                    ).scalar()
                    or 0
                ),
                2,
        ...
        # api_logs_result = await db.execute(select(TrafficLog).filter(TrafficLog.path.like('/api/v1/%')))
        api_stats = {
            "total_requests": (
                await db.execute(
                    select(func.count())
                    .select_from(TrafficLog)
                    .filter(TrafficLog.path.like("/api/v1/%"))
                )
            ).scalar_one(),
            "error_requests": (
                await db.execute(
                    select(func.count()).filter(
                        TrafficLog.path.like("/api/v1/%"), TrafficLog.status_code >= 400
                    )
                )
            ).scalar_one(),
            "avg_duration": round(
                float(
                    (
                        await db.execute(
                            select(func.avg(TrafficLog.duration_ms)).filter(
                                TrafficLog.path.like("/api/v1/%")
                            )
                        )
                    ).scalar()
                    or 0
        ...
        for endpoint in [
            ...
            # endpoint_logs_result = await db.execute(select(TrafficLog).filter(TrafficLog.path.like(f'{path}%')))
            endpoint_stats[endpoint] = {
                "total": (
                    await db.execute(
                        select(func.count())
                        .select_from(TrafficLog)
                        .filter(TrafficLog.path.like(f"{path}%"))
                    )
                ).scalar_one(),
                "errors": (
                    await db.execute(
                        select(func.count()).filter(
                            TrafficLog.path.like(f"{path}%"),
...
async def export_logs(
    ...
    try:
        ...
        logs = await TrafficLog.get_recent_logs(db, limit=None)  # None to get all logs
