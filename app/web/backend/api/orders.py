...
@orders_router.post(
    "/placeorder",
    summary="Place an order with the broker",
...
async def place_order_endpoint(
    ...
    api_key: Optional[
        str
    ] = None,  # Assuming API key can be passed as a header or part of the body
    current_user: dict = Depends(
        check_session_validity_fastapi
...
async def place_smart_order_endpoint(
    ...
    success, response_data, status_code = await place_smart_order(
        order_data=order_data.model_dump(), api_key=api_key
    )
...
async def close_position_endpoint(
    ...
    success, response_data, status_code = await close_position(
        position_data=position_data.model_dump(), api_key=api_key
    )
...
@orders_router.post(
    "/basketorder",
    summary="Place multiple orders in a basket",
    responses={status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": BaseErrorResponse}},
)
async def basket_order_endpoint(
    ...
    success, response_data, status_code = await place_basket_order(
        basket_data=basket_data.model_dump(), api_key=api_key
    )
...
@orders_router.post(
    "/splitorder",
    summary="Split a large order into multiple orders of specified size",
    responses={status.HTTP_500_INTERNAL_SERVER_ERROR: {"model": BaseErrorResponse}},
)
async def split_order_endpoint(
    ...
    success, response_data, status_code = await split_order(
        split_data=split_data.model_dump(), api_key=api_key
    )
...
async def order_status_endpoint(
    ...
    success, response_data, status_code = await get_order_status(
        status_data=status_data.model_dump(), api_key=api_key
    )
