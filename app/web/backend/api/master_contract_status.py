...
master_contract_status_router = APIRouter(
    prefix="/api/master-contract", tags=["Master Contract"]
)
...
async def get_master_contract_status(
    ...
    try:
        ...
        if not broker:
            return JSONResponse(
                {"status": "error", "message": "No broker session found"},
                status_code=401,
            )

        status_data = await get_status(db, broker)
        return JSONResponse(status_data)
    except Exception as e:
        logger.error(f"Error getting master contract status: {e}")
        return JSONResponse(
            {"status": "error", "message": "Failed to get master contract status"},
            status_code=500,
        )
...
async def check_master_contract_ready(
    ...
    try:
        ...
        if not broker:
            return JSONResponse(
                {"ready": False, "message": "No broker session found"}, status_code=401
            )
