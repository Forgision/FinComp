...
class BotConfig(BaseModel):
    ...
    broadcast_enabled: Optional[bool] = Field(
        None, description="Enable broadcast messages"
    )
...
class Broadcast(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(
        None, description="Optional filters for users"
    )
...
class UserPreferences(BaseModel):
    apikey: str = Field(..., description="API Key")
    telegram_id: int = Field(..., description="Telegram User ID")
    order_notifications: Optional[bool] = Field(
        None, description="Enable order notifications"
    )
    trade_notifications: Optional[bool] = Field(
        None, description="Enable trade notifications"
    )
    pnl_notifications: Optional[bool] = Field(
        None, description="Enable P&L notifications"
    )
...
async def get_api_key(
    x_api_key: Optional[str] = Depends(None), apikey: Optional[str] = None
):
...
async def update_bot_configuration(
    config_data: BotConfig, api_key: str = Depends(get_api_key)
):
    """
...
async def broadcast_message(
    broadcast_data: Broadcast, api_key: str = Depends(get_api_key)
):
    """
    ...
    try:
        ...
        if not config.get("broadcast_enabled", True):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Broadcast is disabled"
            )
        ...
        success_count, fail_count = await telegram_bot_service.broadcast_message(
            broadcast_data.message, broadcast_data.filters
        )
...
async def send_notification(
    notification_data: Notification, api_key: str = Depends(get_api_key)
):
    """
    ...
    try:
        ...
        user = await asyncio.to_thread(
            get_telegram_user_by_username, notification_data.username
        )
        if not user:
            raise HTTPException(
...
async def get_user_telegram_preferences(
    api_key: str = Depends(get_api_key),
...
async def update_user_telegram_preferences(
    preferences_data: UserPreferences, api_key: str = Depends(get_api_key)
):
    """
    ...
    try:
        ...
        success = await asyncio.to_thread(
            update_user_preferences, preferences_data.telegram_id, preferences_update
        )
        if success:
