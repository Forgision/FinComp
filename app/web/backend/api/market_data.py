...
market_data_router = APIRouter(
    ...
    tags=["Market Data"],
    dependencies=[
        Depends(check_session_validity_fastapi)
    ],  # Secure all market data endpoints
)
...
async def get_intervals_endpoint(
    intervals_data: IntervalsSchema,
    current_user: dict = Depends(check_session_validity_fastapi),
):
    """
    ...
    """
    try:
        success, response_data, status_code = await get_intervals(
...
async def get_expiry_endpoint(
    expiry_data: ExpirySchema,
    current_user: dict = Depends(check_session_validity_fastapi),
):
    """
...
async def get_ticker_endpoint(
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            symbol = (
                symbol_with_exchange  # Assuming symbol_with_exchange is just the symbol
            )
        ...
        if history_schema.start_date and history_schema.end_date:
            ...
            if was_adjusted:
                logger.info(
                    f"Date range restricted for {history_schema.symbol} ({history_schema.interval}): {adjusted_start} to {adjusted_end}"
                )
        ...
        try:
            ...
            if response_format == "txt":
                ...
                for _, row in df.iterrows():
                    timestamp = convert_timestamp(
                        row["timestamp"], history_schema.interval
                    )
                    volume = int(row["volume"])
                    if history_schema.interval.upper() == "D":
                        text_output.append(
                            f"{symbol_for_output},{timestamp},{row['open']},{row['high']},{row['low']},{row['close']},{volume}"
                        )
                    else:
                        date, time = timestamp
                        text_output.append(
                            f"{symbol_for_output},{date},{time},{row['open']},{row['high']},{row['low']},{row['close']},{volume}"
                        )
                ...
                return JSONResponse(
                    content={"status": "success", "data": df.to_dict(orient="records")},
            ...
            if response_format == "txt":
                return PlainTextResponse(
                    BaseErrorResponse(message=str(e)).model_dump_json(),
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=BaseErrorResponse(message=str(e)).model_dump(),
            )
