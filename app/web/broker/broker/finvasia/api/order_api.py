...
from broker.finvasia.mapping.transform_data import (
    map_product_type,
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    net_qty = '0'
    if positions_data is None or (isinstance(positions_data, dict) and (positions_data.get('stat') == "Not_Ok")):
...
def cancel_all_orders_api(data, auth):
    ...
    if order_book_response is None:
        return [], []
    orders_to_cancel = [order for order in order_book_response if order.get('status') in ['OPEN', 'TRIGGER PENDING']]
    canceled_orders = []
