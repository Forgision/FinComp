...
class BrokerData:
    ...
    def _get_instrument_key(self, symbol: str, exchange: str) -> str:
        ...
        try:
            # Get token from app.core.schemas - this already includes the exchange prefix
            ...
            if not token and exchange in ["NSE", "BSE", "MCX"]:
                ...
                logger.debug(
                    f"Token not found for {symbol} on {exchange}, trying {index_exchange}"
                )
            ...
            if not token and exchange.endswith("_INDEX"):
                ...
                logger.debug(
                    f"Token not found for {symbol} on {exchange}, trying {base_exchange}"
                )
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            logger.debug(
                f"get_quotes called with symbol='{symbol}', exchange='{exchange}'"
            )
