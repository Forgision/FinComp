...
from app.core.brokers.pocketful.mapping.transform_data import (
    map_product_type,
    ...
    if not client_id:
        ...
        if info_response.get("status") == "success":
            ...
            if client_id and username:
                auth_obj = (
                    db.query(Auth).filter_by(name=username, broker="pocketful").first()
                )
                if auth_obj:
...
def get_holdings(auth):
    ...
    from app.core.brokers.pocketful.mapping.order_data import transform_holdings_data
