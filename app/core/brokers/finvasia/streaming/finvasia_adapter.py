...
class MarketDataCache:
    ...
    def _log_cache_initialization(self, token: str, data: Dict) -> None:
        ...
        self.logger.info(
            f"Initializing cache for token {token} - {present_fields}/{len(basic_fields)} fields present ({completeness:.1%})"
        )
...
class FinvasiaWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def initialize(
        ...
        self.actid = (
            api_key[:-2] if api_key and len(api_key) > 2 else api_key or user_id
        )
    ...
    def disconnect(self) -> None:
        ...
        self.connected = False
        self.logger.info(
            "Disconnected from Finvasia WebSocket and cleaned up all resources"
        )
    ...
    def _remove_subscription(self, correlation_id: str, subscription: Dict) -> None:
        ...
        if correlation_id in self.subscriptions:
            del self.subscriptions[correlation_id]
        if (
            scrip in self.ws_subscription_refs
            and self.ws_subscription_refs[scrip]["touchline_count"] <= 0
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        self.logger.info(
            f"Finvasia WebSocket connection closed: {close_status_code} - {close_msg}"
        )
        self.connected = False
        if self.running:
    ...
    def _resubscribe_all(self):
        with self.lock:
            ...
            if touchline_scrips:
                ...
                self.logger.info(
                    f"Resubscribed to {len(touchline_scrips)} touchline scrips"
                )
            if depth_scrips:
    ...
    def _find_matching_subscriptions(self, token: str) -> List[Dict]:
        with self.lock:
            return [
                sub for sub in self.subscriptions.values() if sub.get("token") == token
            ]
