...
def get_kotak_master_filepaths():
    ...
    if not login_username:
        ...
        socketio.emit(
            ...
            {"status": "error", "message": "API_CLIENT not set in settings."},
        )
        return {}
    ...
    if not auth_token:
        ...
        socketio.emit(
            ...
            {
                "status": "error",
                "message": f"Auth token not found for user: {login_username}",
            },
        )
        return {}
...
def search_symbols(symbol, exchange):
    ...
    return (
        db.query(SymToken)
        .filter(SymToken.symbol.like(f"%{symbol}%"), SymToken.exchange == exchange)
        .all()
    )
