...
async def get_holdings_with_auth(
    ...
    try:
        ...
        holdings_func = broker_funcs["get_holdings"]
        if inspect.iscoroutinefunction(holdings_func):
            holdings = await holdings_func(auth_token)
        else:
            holdings = holdings_func(auth_token)

        # If the result is a coroutine (async function not detected by iscoroutinefunction because it might be wrapped), await it
        if inspect.iscoroutine(holdings):
