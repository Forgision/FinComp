...
async def emit_analyzer_error(
    ...
    await async_log_analyzer(db, analyzer_request, error_response, "cancelorder")
...
async def cancel_order(
    ...
    if api_key and not (auth_token and broker):
        ...
        return await cancel_order_with_auth(
            db, orderid, AUTH_TOKEN, broker_name, original_data
        )

    # Case 2: Direct internal call with auth_token and broker
    elif auth_token and broker:
        return await cancel_order_with_auth(
            db, orderid, auth_token, broker, original_data
        )
