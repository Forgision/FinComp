...
import telegram  # Added for exception handling
...
class TelegramBotService:
    ...
    async def _make_sdk_call(
        self, telegram_id: int, method: str, **kwargs
    ) -> Optional[Dict]:
    ...
    async def _generate_daily_chart(
        ...
        try:
            ...
            start_date = end_date - timedelta(
                days=365
            )  # Get more days to ensure 252 trading days
    ...
    async def initialize_bot(
        self, token: str, webhook_url: Optional[str] = None
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if intraday_chart and daily_chart:
                ...
                media = [
                    InputMediaPhoto(
                        intraday_chart,
                        caption=f"{symbol} - 5 Day Intraday Chart (5min intervals)",
                    ),
                    InputMediaPhoto(
                        daily_chart,
