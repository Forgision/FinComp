...
async def split_order_with_auth(
    ...
    try:
        ...
        if split_size <= 0:
            ...
            if get_analyze_mode() is True:
                return (
                    False,
                    await emit_analyzer_error(db, split_request_data, error_message),
                    400,
                )
            error_response = {"status": "error", "message": error_message}
            await async_log_order("splitorder", original_data, error_response)
        ...
        if total_orders > MAX_ORDERS:
            error_message = (
                f"Total number of orders would exceed maximum limit of {MAX_ORDERS}"
            )
            if get_analyze_mode() is True:
                return (
                    False,
        ...
        if get_analyze_mode() is True:
            return (
                False,
                await emit_analyzer_error(db, original_data, error_message),
                400,
            )
        error_response = {"status": "error", "message": error_message}
        await async_log_order("splitorder", original_data, error_response)
    ...
    if get_analyze_mode() is True:
                ...
                analyze_results.append(
                    {
                        "order_num": i + 1,
                        "quantity": (
                            split_size if i < num_full_orders else remaining_qty
                        ),
                        "status": "success",
        ...
        await telegram_alert_service.send_order_alert(
            db, "splitorder", split_data, response_data, split_data.get("apikey")
        )
    ...
    await async_log_order("splitorder", split_request_data, response_data)
...
async def split_order(
    ...
    if api_key and not (auth_token and broker):
        ...
        return await split_order_with_auth(
            db, split_data, AUTH_TOKEN, broker_name, original_data
        )

    # Case 2: Direct internal call with auth_token and broker
    elif auth_token and broker:
        return await split_order_with_auth(
            db, split_data, auth_token, broker, original_data
        )
