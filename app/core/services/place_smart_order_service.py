...
def validate_smart_order(order_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if (
        "product_type" in order_data
        and order_data["product_type"] not in VALID_PRODUCT_TYPES
    ):
        return (
            False,
...
async def place_smart_order_with_auth(
    db: AsyncSession,
        ...
        if get_analyze_mode() is True:
            return (
                ...
                await emit_analyzer_error(
                    db, original_data, error_message or "Validation failed"
                ),
                400,
            )
        error_response = {
    ...
    if get_analyze_mode() is True:
        ...
        if not api_key:
            return (
                False,
                await emit_analyzer_error(
                    db, original_data, "API key required for sandbox mode"
                ),
                400,
            )

        # Route to sandbox smart order
        success, response_data, status_code = await sandbox_place_smart_order(
        ...
        await telegram_alert_service.send_order_alert(
            db, "placesmartorder", order_data, response_data, order_data.get("apikey")
        )
    ...
    try:
        ...
        if (
            ...
            await async_log_order(
                "placesmartorder", order_request_data, order_response_data
            )
    ...
    # Add delay if needed
    try:
        await asyncio.sleep(float(smart_order_delay))
    except Exception:
...
async def place_smart_order(
    ...
    if api_key and not (auth_token and broker):
        ...
        return await place_smart_order_with_auth(
            db, order_data, AUTH_TOKEN, broker_name, original_data, smart_order_delay
        )

    # Case 2: Direct internal call with auth_token and broker
    elif auth_token and broker:
        return await place_smart_order_with_auth(
            db, order_data, auth_token, broker, original_data, smart_order_delay
        )
