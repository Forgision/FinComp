...
async def is_sandbox_mode(db: AsyncSession) -> bool:
    """Check if sandbox/analyzer mode is enabled"""
    return get_analyze_mode() is True


async def get_user_id_from_apikey(db: AsyncSession, api_key: str) -> Optional[str]:
...
async def sandbox_place_order(
    ...
    try:
        # Get user ID from API key
        user_id = await get_user_id_from_apikey(db, api_key)
        if not user_id:
            return (
                False,
                {"status": "error", "message": "Invalid API key", "mode": "analyze"},
                403,
            )
        ...
        sandbox_order_data = {
            ...
            or order_data.get("price_type", "MARKET"),
            "product": order_data.get("product")
            or order_data.get("product_type", "MIS"),
        ...
        # Emit socket event
        await sio.emit(
            "analyzer_update", {"request": log_request, "response": response}
        )

        # Send Telegram alert
        await telegram_alert_service.send_order_alert(
            db, "placeorder", order_data, response, api_key
        )
...
async def sandbox_get_orderbook(
    db: AsyncSession, api_key: str, original_data: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any], int]:
...
async def sandbox_get_order_status(
    db: AsyncSession,
    order_data: Dict[str, Any],
    api_key: str,
    original_data: Dict[str, Any],
...
async def sandbox_get_positions(
    db: AsyncSession, api_key: str, original_data: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any], int]:
    ...
    try:
        ...
        success, response, status_code = position_manager.get_open_positions(
            update_mtm=True
        )
...
async def sandbox_get_holdings(
    db: AsyncSession, api_key: str, original_data: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any], int]:
...
async def sandbox_get_tradebook(
    db: AsyncSession, api_key: str, original_data: Dict[str, Any]
) -> Tuple[bool, Dict[str, Any], int]:
...
async def sandbox_close_position(
    ...
    try:
        ...
        if not symbol and not exchange:
            ...
            success, positions_response, status_code = (
...
async def sandbox_place_smart_order(
    ...
    try:
        ...
        if target_quantity == 0 and current_quantity == 0 and original_quantity != 0:
            ...
            return (
                True,
                {"status": "success", "message": message, "mode": "analyze"},
                200,
            )
...
async def sandbox_cancel_all_orders(
    ...
    try:
        ...
        open_orders = [
            ...
            and order.get("order_status") in ["open", "pending", "trigger_pending"]
        ]
...
def sandbox_reload_squareoff_schedule() -> Tuple[bool, Dict[str, Any], int]:
    ...
    try:
        ...
        if success:
            ...
            return (
                False,
                {"status": "error", "message": message, "mode": "analyze"},
                500,
            )
