...
class TelegramBotService:
    ...
    def _get_sdk_client(
        ...
        try:
            ...
            credentials = get_user_credentials(session, telegram_id)
    ...
    async def _make_sdk_call(
        self, session: AsyncSession, telegram_id: int, method: str, **kwargs
    ) -> Optional[Dict]:
        """Make an SDK call in async context"""
        try:
            client = self._get_sdk_client(session, telegram_id)
            if not client:
                return None
    ...
    async def _generate_intraday_chart(
        ...
        try:
            ...
            # Convert to image bytes in a thread pool to avoid blocking the event loop
            loop = asyncio.get_event_loop()
            img_bytes = await loop.run_in_executor(
                None, lambda: fig.to_image(format="png", engine="kaleido")
            )
            return img_bytes
    ...
    async def _generate_daily_chart(
        ...
        try:
            ...
            tick_spacing = max(
                1, len(df) // 10
            )  # Show approximately 10 ticks for daily
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("ok"):
                        ...
                        self.bot_token = token
                        # Run async update_bot_config in a new thread
                        config_data = {
                            "bot_token": token,
                            "is_active": False,
                        ...
                        async def _update_bot_config_async(data):
                            async with AsyncSessionLocal() as session:
                                await update_bot_config(session, data)

                        update_thread = original_threading.Thread(
                            target=_run_async_in_thread,
                            args=(_update_bot_config_async(config_data),),
                        )
                        update_thread.start()
                        update_thread.join()  # Wait for the update to complete
    ...
    async def handle_error(
        ...
        if isinstance(error, telegram.error.NetworkError):
            ...
            logger.error(
                "Another instance of the bot is running! Please stop other instances."
            )
            self.is_running = False
        ...
        while retry_count < max_retries:
            try:
                ...
                self.application.add_handler(
                    CommandHandler("orderbook", self.cmd_orderbook)
                )
                self.application.add_handler(
                    CommandHandler("tradebook", self.cmd_tradebook)
                )
                self.application.add_handler(
                ...
                logger.info(
                    "Telegram bot started successfully and is polling for updates"
                )
                ...
                if retry_count < max_retries:
                    ...
                    logger.error(
                        "Max retries reached. Unable to connect to Telegram servers."
                    )
                    logger.info(
    ...
    def start_bot(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            async def _get_bot_config_async():
                async with AsyncSessionLocal() as session:
                    return await get_bot_config(session)

            config_result = []
            # Get bot config in a new thread
            config_thread = original_threading.Thread(
                target=_run_async_in_thread,
                args=(_get_bot_config_async(), config_result),
            )
            config_thread.start()
            config_thread.join()  # Wait for the config to be fetched
    ...
    def stop_bot(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            async def _update_bot_config_async():
                async with AsyncSessionLocal() as session:
                    await update_bot_config(session, {"is_active": False})

            update_thread = original_threading.Thread(
                target=_run_async_in_thread, args=(_update_bot_config_async(),)
            )
            update_thread.start()
            update_thread.join()
    ...
    async def cmd_start(
        ...
        async with AsyncSessionLocal() as session:
            telegram_user = await get_telegram_user(session, user.id)
        ...
        await log_command(session, user.id, "start", update.effective_chat.id)
...
"""
        ...
        await log_command(
            session, update.effective_user.id, "help", update.effective_chat.id
        )
    ...
    async def cmd_link(
            ...
            if test_response and test_response.get("status") == "success":
                ...
                try:
                    with AsyncSessionLocal() as session:
                        openalgo_username = await get_username_by_apikey(
                            session, api_key
                        )
                ...
                if not openalgo_username and test_response.get("data"):
                    ...
                    if isinstance(data, dict):
                        ...
                        if openalgo_username:
                            logger.info(
                                f"Got username from funds response: {openalgo_username}"
                            )
                ...
                if not openalgo_username:
                    ...
                    logger.info(
                        f"Successfully retrieved OpenAlgo username: {openalgo_username}"
                    )

                await create_or_update_telegram_user(
                    session,
                    telegram_id=user.id,
                ...
                logger.info(
                    f"Database updated - Username stored as: {openalgo_username}"
                )
        ...
        await log_command(session, user.id, "link", chat_id)
    ...
    async def cmd_unlink(
        ...
        user = update.effective_user
        async with AsyncSessionLocal() as session:
            if await delete_telegram_user(session, user.id):
            ...
            await log_command(session, user.id, "unlink", update.effective_chat.id)
    ...
    async def cmd_status(
        ...
        await log_command(session, user.id, "status", update.effective_chat.id)

    async def cmd_orderbook(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        ...
        async with AsyncSessionLocal() as session:
            ...
            client = self._get_sdk_client(session, user.id)
        ...
        await log_command(session, user.id, "orderbook", update.effective_chat.id)

    async def cmd_tradebook(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "tradebook", update.effective_chat.id)
    ...
    async def cmd_positions(
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "positions", update.effective_chat.id)
    ...
    async def cmd_holdings(
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "holdings", update.effective_chat.id)
    ...
    async def cmd_funds(
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "funds", update.effective_chat.id)
    ...
    async def cmd_pnl(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "pnl", update.effective_chat.id)
    ...
    async def cmd_quote(
        ...
        await update.message.reply_text(message, parse_mode=ParseMode.MARKDOWN)
        await log_command(session, user.id, "quote", update.effective_chat.id)
    ...
    async def cmd_chart(
            ...
            try:
                ...
                if chart_type in ["both", "daily", "d"]:
                    ...
                    daily_chart = await self._generate_daily_chart(
                        session, symbol, exchange, daily_interval, daily_days, user.id
                    )
                    if daily_chart:
                        charts_generated.append(
                            InputMediaPhoto(
                ...
                if charts_generated:
                    ...
                    await update.message.reply_text(
                        f"❌ Failed to generate charts for {symbol}"
                    )
        ...
        await log_command(session, user.id, "chart", update.effective_chat.id)
    ...
    async def cmd_menu(
        ...
        async with AsyncSessionLocal() as session:
            ...
            await log_command(session, user.id, "menu", update.effective_chat.id)
    ...
    async def broadcast_message(
        self, message: str, filters: Dict = None
    ) -> Tuple[int, int]:
        ...
        try:
            ...
            if not users:
                logger.error("No users found for broadcast")
                raise ValueError("No users found for broadcast")
                return 0, 0
            ...
            if filters:
                ...
                if filters.get("notifications_enabled") is not None:
                    users = [
                        u
                        for u in users
                        if u.get("notifications_enabled")
