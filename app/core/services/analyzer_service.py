...
async def get_analyzer_status(db: AsyncSession, analyzer_data: dict, api_key: str):
    ...
    try:
        is_enabled = await get_analyze_mode(db)
...
async def toggle_analyzer_mode(db: AsyncSession, analyzer_data: dict, api_key: str):
    ...
    try:
        current_mode = await get_analyze_mode(db)
        new_mode = not current_mode
        await set_analyze_mode(db, new_mode)
        response_data = {
            "status": "success",
...
async def get_filtered_requests(db: AsyncSession, start_date=None, end_date=None):
    ...
    try:
        ...
        results = (
            (await db.execute(stmt.order_by(AnalyzerLog.created_at.desc()))).scalars().all()
        )
        requests = []
