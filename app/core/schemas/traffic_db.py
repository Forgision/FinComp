...
class Error404Tracker(LogBase):
    ...
    first_error_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
...
async def get_stats(db: AsyncSession) -> dict:
    try:
        total_requests = (
            await db.execute(select(func.count(TrafficLog.id)))
        ).scalar_one_or_none() or 0
        error_requests = (
            await db.execute(
                select(func.count(TrafficLog.id)).filter(TrafficLog.status_code >= 400)
            )
        ).scalar_one_or_none() or 0
        avg_duration = (
...
async def get_all_bans(db: AsyncSession) -> List[IPBan]:
    try:
        stmt_delete = select(IPBan).filter(
            IPBan.is_permanent.is_(False), IPBan.expires_at < datetime.utcnow()
        )
...
async def get_suspicious_ips(
    db: AsyncSession, min_errors: int = 5
) -> List[Error404Tracker]:
    try:
        cutoff = datetime.utcnow() - timedelta(days=1)
        stmt_delete = select(Error404Tracker).filter(
...
async def track_invalid_api_key(
    db: AsyncSession, ip_address: str, api_key_hash: Optional[str] = None
) -> bool:
    try:
        if await is_ip_banned(db, ip_address):
            return False
...
async def get_suspicious_api_users(
    db: AsyncSession, min_attempts: int = 3
) -> List[InvalidAPIKeyTracker]:
    try:
        cutoff = datetime.utcnow() - timedelta(days=1)
        stmt_delete = select(InvalidAPIKeyTracker).filter(
