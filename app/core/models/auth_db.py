...
class Auth(SQLModel, table=True):
    ...
    auth: str
    feed_token: Optional[str] = Field(default=None)
    broker: str = Field(max_length=20)
...
def upsert_auth(db: Session, name: str, auth_token: str, broker: str, feed_token: Optional[str] = None, user_id: Optional[str] = None, revoke: bool = False):
    encrypted_token = encrypt_token(auth_token)
    encrypted_feed_token = encrypt_token(feed_token) if feed_token else None

    statement = select(Auth).where(Auth.name == name)
    auth_obj = db.exec(statement).first()
...
def get_auth_token(db: Session, name: str) -> Optional[str]:
    if not name:
...
def get_feed_token(db: Session, name: str) -> Optional[str]:
    if not name:
...
def upsert_api_key(db: Session, user_id: str, api_key: str):
    ...
    statement = select(ApiKeys).where(ApiKeys.user_id == user_id)
    api_key_obj = db.exec(statement).first()
    ...
    if api_key_obj:
        ...
        api_key_obj = ApiKeys(user_id=user_id, api_key_hash=hashed_key, api_key_encrypted=encrypted_key)
        db.add(api_key_obj)
    db.commit()
...
def verify_api_key(db: Session, provided_api_key: str) -> Optional[str]:
    ...
    from app.core.models.traffic_db import track_invalid_api_key
...
def get_broker_name(db: Session, provided_api_key: str) -> Optional[str]:
    ...
    if user_id:
        statement = select(Auth).where(Auth.name == user_id)
        auth_obj = db.exec(statement).first()
        if auth_obj and not auth_obj.is_revoked:
...
def get_auth_token_broker(db: Session, provided_api_key: str, include_feed_token: bool = False):
    user_id = verify_api_key(db, provided_api_key)
