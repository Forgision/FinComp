...
class HealthCheckResponse(BaseModel):
    ...
    dependencies: dict | None = Field(
        None, description="Status of key dependencies (e.g., database, external APIs)."
    )
    uptime_seconds: float | None = Field(
        None, description="Uptime of the service in seconds."
    )
...
class PerformanceMetric(BaseModel):
    ...
    unit: str | None = Field(
        None, description="Unit of the metric (e.g., 'ms', 'req/s')."
    )
    tags: dict | None = Field(
        None, description="Optional tags for filtering/grouping metrics."
    )
