...
class HoldingsManager:
    ...
    async def get_holdings(self, update_mtm=True):
        """
        ...
        try:
            async with AsyncSessionLocal() as db_session:
                ...
                if update_mtm:
                    await self._update_holdings_mtm(holdings)
                ...
                for holding in holdings:
                    ...
                    current_value = (
                        abs(holding.quantity) * holding.ltp
                        if holding.ltp
                        else Decimal("0.00")
                    )
                    ...
                    holdings_list.append(
                        {
                            ...
                            "settlement_date": holding.settlement_date.strftime(
                                "%Y-%m-%d"
                            ),
                        }
                    )
    ...
    async def process_t1_settlement(self):
        """
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                stmt = (
                    ...
                    .filter_by(user_id=self.user_id, product="CNC")
                    .filter(SandboxPositions.created_at < settlement_cutoff)
                )
                result = await db_session.execute(stmt)
                cnc_positions = result.scalars().all()
                ...
                for position in cnc_positions:
                    ...
                    if holding:
                        ...
                        if position.quantity > 0:
                            ...
                            total_value = (
                                abs(holding.quantity) * holding.average_price
                            ) + (abs(position.quantity) * position.average_price)
                            total_quantity = abs(holding.quantity) + abs(
                                position.quantity
                            )
                        ...
                        if holding.quantity == 0:
                            ...
                            logger.info(
                                f"Deleted zero-quantity holding: {position.symbol}"
                            )
                ...
                logger.info(
                    f"Settled {settled_count} CNC positions for user {self.user_id}"
                )
                ...
                logger.error(
                    f"Error processing T+1 settlement for user {self.user_id}: {e}"
                )
                return False, f"Settlement error: {str(e)}"

    async def _update_holdings_mtm(self, holdings):
...
async def process_all_t1_settlements():
    ...
    async with AsyncSessionLocal() as db_session:
        try:
            ...
            for user_id in users:
                ...
                success, message = await hm.process_t1_settlement()
                if success:
                    settled_users += 1
...
if __name__ == "__main__":
    ...
    asyncio.run(process_all_t1_settlements())
