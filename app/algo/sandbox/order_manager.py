...
class OrderManager:
    ...
    async def place_order(self, order_data):
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                symbol_obj = (await db.execute(stmt)).scalars().first()
                if not symbol_obj:
                    return (
                        False,
                        {
                ...
                if (
                    exchange in ["NFO", "BFO", "CDS", "BCD", "MCX", "NCDEX"]
                    and symbol_obj.lotsize is not None
                ):
                        ...
                        if is_blocked:
                            ...
                            if existing_position:
                                if action == "BUY" and existing_position.quantity < 0:
                                    ...
                                    action == "SELL" and existing_position.quantity > 0
                                ):
                ...
                if action == "SELL":
                    if product == "CNC":
                        ...
                        existing_position = (await db.execute(stmt)).scalars().first()
                        ...
                        existing_holdings = (await db.execute(stmt)).scalars().first()
                        ...
                        logger.info(
                            f"MIS SELL order: {symbol} - Short selling allowed for intraday"
                        )
                ...
                if price_type == "MARKET":
                    ...
                    try:
                        ...
                        quote = await engine._fetch_quote(symbol, exchange)
                        if quote and quote.get("ltp"):
                            ...
                            if existing_position and existing_position.ltp:
                                ...
                                logger.warning(
                                    f"API failed, using last known price {margin_calculation_price} for {symbol}"
                                )
                            else:
                                # Use a reasonable default for sandbox testing
                                margin_calculation_price = Decimal(
                                    "112.37"
                                )  # Default ZEEL price for testing
                                logger.warning(
                                    f"API failed, using default sandbox price {margin_calculation_price} for {symbol}"
                                )
                    except Exception as e:
                        logger.error(
                            f"Error fetching quote for margin calculation: {e}"
                        )
                        # In sandbox mode, use a fallback price
                        if existing_position and existing_position.ltp:
                            margin_calculation_price = existing_position.ltp
                            logger.warning(
                                f"API error, using last known price {margin_calculation_price} for {symbol}"
                            )
                        else:
                            margin_calculation_price = Decimal(
                                "112.37"
                            ...
                            logger.warning(
                                f"API error, using default sandbox price {margin_calculation_price} for {symbol}"
                            )
                    ...
                    logger.debug(
                        f"Using LIMIT price {margin_calculation_price} for margin calculation"
                    )
                ...
                if existing_position and existing_position.quantity != 0:
                    ...
                    if (existing_position.quantity > 0 and action == "SELL") or (
                        ...
                        if order_qty <= existing_qty:
                            ...
                            logger.info(
                                "Order will reduce position - no margin required"
                            )
                        else:
                ...
                if should_block_margin:
                    if (
                        actual_margin_to_block is not None
                        and actual_margin_to_block > 0
                    ):
                        # Check and block margin only for new exposure
                        can_trade, margin_check_msg = (
                            self.fund_manager.check_margin_available(
                ...
                orderid = await self._generate_order_id(db)
                ...
                if cnc_sell_rejection_reason:
                    ...
                    order_price_to_store = (
                        margin_calculation_price if price_type == "MARKET" else price
                    )
                    ...
                    order = SandboxOrders(
                        ...
                        margin_blocked=Decimal(
                            "0"
                        ),  # No margin blocked for rejected orders
                ...
                logger.info(
                    f"Order placed: {orderid} - {symbol} {action} {quantity} @ {price_type}"
                )
                ...
                if price_type == "MARKET":
                    try:
                        ...
                        if quote:
                            ...
                            await engine._process_order(order, quote)
                ...
                return (
                    True,
                    {"status": "success", "orderid": orderid, "mode": "analyze"},
                    200,
                )
    ...
    async def modify_order(self, orderid, new_data):
        """
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                stmt = select(SandboxOrders).filter_by(
                    orderid=orderid, user_id=self.user_id
                )
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                if (
                    ...
                    and order.margin_blocked is not None
                        ...
                        if should_release_margin:
                            ...
                            if not order.price:
                                ...
                                try:
                                    ...
                                    engine = ExecutionEngine()
                                    quote = await engine._fetch_quote(
                                        order.symbol, order.exchange
                                    )
                                    if quote and quote.get("ltp"):
                                    ...
                                    logger.error(
                                        f"Error fetching quote for margin release: {e}"
                                    )
                                    order_price = Decimal("0")
                            else:
                    ...
                    orderbook.append(
                        {
                            ...
                            "timestamp": order.order_timestamp.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
    ...
    async def _generate_order_id(self, db):
        """
        ...
        today_orders = (await db.execute(stmt)).scalar_one()
