...
class FundManager:
    ...
    async def initialize_funds(self):
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                stmt = select(SandboxFunds).filter_by(user_id=self.user_id)
                result = await db_session.execute(stmt)
                funds = result.scalars().first()
        ...
        try:
            async with AsyncSessionLocal() as db_session:
                ...
                if not funds:
                    ...
                    success, message = await self.initialize_funds()
                    if not success:
                        return None
                ...
                await self._check_and_reset_funds(funds)
    ...
    async def _check_and_reset_funds(self, funds):
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                await db_session.execute(
                    delete(SandboxPositions).filter_by(user_id=self.user_id)
                )
                await db_session.execute(
                    delete(SandboxHoldings).filter_by(user_id=self.user_id)
                )
                await db_session.commit()
    ...
    async def check_margin_available(self, required_margin):
    ...
    async def block_margin(self, amount, description=""):
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                logger.info(
                    f"Blocked ₹{amount} margin for user {self.user_id}. {description}"
                )
    ...
    async def release_margin(
        self, amount, realized_pnl: Decimal = Decimal("0"), description=""
    ):
    ...
    async def transfer_margin_to_holdings(self, amount, description=""):
        """
    ...
    async def credit_sale_proceeds(self, amount, description=""):
        """
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                await db_session.rollback()
                logger.error(
                    f"Error crediting sale proceeds for user {self.user_id}: {e}"
                )
                return False, f"Error crediting sale proceeds: {str(e)}"

    async def update_unrealized_pnl(self, unrealized_pnl):
        ...
        async with AsyncSessionLocal() as db_session:
            try:
                ...
                await db_session.rollback()
                logger.error(
                    f"Error updating unrealized P&L for user {self.user_id}: {e}"
                )
                return False, f"Error updating unrealized P&L: {str(e)}"

    async def calculate_margin_required(
        self, symbol, exchange, product, quantity, price, action=None
    ):
...
async def get_user_funds(user_id):
    """Helper function to get user funds"""
    fund_manager = FundManager(user_id)
    return await fund_manager.get_funds()


async def initialize_user_funds(user_id):
    """Helper function to initialize user funds"""
    fund_manager = FundManager(user_id)
    return await fund_manager.initialize_funds()


async def reset_all_user_funds():
    """
    ...
    async with AsyncSessionLocal() as db_session:
        try:
            logger.info(
                "=== AUTO-RESET: Starting scheduled fund reset for all users ==="
            )

            # Get all unique user IDs from funds table
            stmt = select(SandboxFunds)
            result = await db_session.execute(stmt)
            ...
            logger.info(
                f"=== AUTO-RESET: Successfully reset {reset_count} user fund accounts ==="
            )
