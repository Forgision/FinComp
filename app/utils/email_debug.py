...
async def debug_smtp_connection(db: AsyncSession):
    """
    Debug SMTP connection with detailed logging.
    Returns detailed connection information for troubleshooting.
    """

    async def _debug_sync(db: AsyncSession, smtp_settings: dict):
        ...
        try:
            ...
            details.append(
                f"🔧 HELO Hostname: {smtp_settings.get('smtp_helo_hostname') or 'default'}"
            )
            ...
            try:
                ...
                if hasattr(server, "ehlo_resp"):
                    details.append(
                        f"📋 Server capabilities: {server.ehlo_resp.decode('utf-8')}"
                    )
        ...
        return await asyncio.to_thread(_debug_sync, db, smtp_settings)


async def test_gmail_configurations(db: AsyncSession):
    """
        ...
        try:
            ...
            if config["port"] == 465:
                server = smtplib.SMTP_SSL(
                    config["server"], config["port"], context=context
                )
            else:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with AsyncSessionLocal() as db_session:
            result = await debug_smtp_connection(db_session)
            ...
            if not result["success"]:
                ...
                print(await test_gmail_configurations(db_session))
