...
def validate_csrf_token(token: str, session_id: str) -> bool:
    ...
    try:
        ...
        secret_key = settings.APP_KEY.encode("utf-8")
        h = hmac.new(
            secret_key, salt + timestamp_str + session_id.encode("utf-8"), "sha256"
        )
...
class CSRFMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp):
        ...
        self.exempt_routes = [
            route.strip()
            for route in settings.CSRF_EXEMPT_ROUTES.split(",")
            if route.strip()
        ]
    ...
    async def dispatch(
        ...
        if request.method in ["GET", "HEAD", "OPTIONS", "TRACE"]:
            ...
            if not validate_csrf_token(csrf_token, session_id):
                logger.warning(
                    f"CSRF validation failed for request to {request.url.path}"
                )
    ...
    def _set_csrf_cookie(self, response: Response, csrf_token: str):
        ...
        response.set_cookie(
            ...
            samesite="lax",
            secure=settings.USE_HTTPS,
            max_age=settings.CSRF_TIME_LIMIT
            if settings.CSRF_TIME_LIMIT is not None
            else 3600 * 24 * 30,  # Default 30 days
        )
