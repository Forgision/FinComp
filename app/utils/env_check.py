...
def check_env_version_compatibility():
    ...
    if not env_version:
        ...
        print(
            "   Your .env file may be outdated and missing new configuration options."
        )
    ...
    try:
        ...
        if env_ver < sample_ver:
            ...
            print("")
            print(
                "   New features may not work properly with an outdated configuration!"
            )
            print("🔴 " + "=" * 68)
...
def load_and_check_env_variables():
    ...
    valid_brokers = set(
        broker.strip().lower() for broker in valid_brokers_str.split(",")
    )
    ...
    if not log_format:
        ...
        print(
            "Example: LOG_FORMAT=[%(asctime)s] %(levelname)s in %(module)s: %(message)s"
        )
        sys.exit(1)
