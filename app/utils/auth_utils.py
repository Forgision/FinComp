...
async def async_master_contract_download(broker, user_id, feed_token):
    ...
    try:
        ...
        update_status(
            broker, "error", f"Failed to import master contract module: {str(error)}"
        )
...
async def handle_auth_success(
    ...
    if inserted_id:
        ...
        thread = Thread(
            target=async_master_contract_download, args=(broker, user_id, feed_token)
        )
        thread.start()
        ...
        return JSONResponse(
            content={
                "error_message": "Failed to store authentication token. Please try again."
            }
        )


async def handle_auth_failure(
    request: Request, error_message, forward_url="broker.html"
):
    """
...
async def get_feed_token(request: Request, db: AsyncSession):
    ...
    if (
        "logged_in" in request.session
        and request.session["logged_in"]
        and "user_session_key" in request.session
    ):
