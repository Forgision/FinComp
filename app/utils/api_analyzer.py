...
async def generate_order_id(db: AsyncSession):
    ...
    try:
        result = await db.execute(
            select(AnalyzerLog)
            .filter(AnalyzerLog.response_data.like('%"orderid": "%"'))
...
async def validate_symbol(db: AsyncSession, symbol: str, exchange: str) -> bool:
    """Validate if symbol exists in the database for given exchange"""
    try:
        result = await db.execute(
            select(SymToken).filter(
...
async def analyze_api_request(db: AsyncSession, order_data):
    ...
    try:
        ...
        if "symbol" in order_data and "exchange" in order_data:
            if not await validate_symbol(
                db, order_data["symbol"], order_data["exchange"]
            ):
                issues.append(
        ...
        try:
            ...
            trigger_price = float(
                order_data.get("trigger_price", DEFAULT_TRIGGER_PRICE)
            )
            disclosed_qty = float(
            ...
            issues.append(
                "Invalid numeric value for price, trigger_price, or disclosed_quantity"
            )
        ...
        try:
            result = await db.execute(
                ...
                .select_from(AnalyzerLog)
                .filter(
                    AnalyzerLog.created_at
                    >= datetime.now(pytz.UTC) - timedelta(minutes=1)
                )
            )
            if result.scalar_one() > 50:
                warnings.append(
...
async def analyze_smart_order_request(db: AsyncSession, order_data):
...
async def analyze_cancel_order_request(db: AsyncSession, order_data):
...
async def analyze_cancel_all_order_request(db: AsyncSession, order_data):
    ...
    try:
        ...
        missing_fields = [
            field
            for field in REQUIRED_CANCEL_ALL_ORDER_FIELDS
...
async def analyze_close_position_request(db: AsyncSession, order_data):
...
async def analyze_modify_order_request(db: AsyncSession, order_data):
...
async def analyze_request(
    db: AsyncSession, request_data, api_type="placeorder", should_log=False
):
    ...
    try:
        ...
        if api_type == "placesmartorder":
            analysis = await analyze_smart_order_request(db, request_data)
        elif api_type == "cancelorder":
            analysis = await analyze_cancel_order_request(db, request_data)
        elif api_type == "cancelallorder":
            analysis = await analyze_cancel_all_order_request(db, request_data)
        elif api_type == "closeposition":
            analysis = await analyze_close_position_request(db, request_data)
        elif api_type == "modifyorder":
            analysis = await analyze_modify_order_request(db, request_data)
        else:
            analysis = await analyze_api_request(db, request_data)
...
async def get_analyzer_stats(db: AsyncSession):
    ...
    try:
        ...
        # Get recent requests
        result = await db.execute(
            select(AnalyzerLog).filter(AnalyzerLog.created_at >= cutoff)
        )
