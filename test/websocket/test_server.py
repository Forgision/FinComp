...
def proxy():
    with (
        patch("app.web.websocket.server.zmq"),
        patch("app.web.websocket.server.is_port_in_use", return_value=False),
    ):
        return WebSocketProxy()


@pytest.mark.asyncio
@patch("app.web.websocket.server.verify_api_key", return_value="test_user")
@patch("app.web.websocket.server.get_broker_name", return_value="finvasia")
@patch("app.web.websocket.server.create_broker_adapter")
async def test_authenticate_client_success(
    mock_create_broker_adapter, mock_get_broker_name, mock_verify_api_key, proxy
):
