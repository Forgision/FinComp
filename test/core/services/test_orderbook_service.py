...
def mock_broker_module():
    ...
    with patch(
        "app.core.services.orderbook_service.import_broker_module"
    ) as mock_import:
        ...
        mock_broker_funcs = {
            ...
            "calculate_order_statistics": Mock(
                side_effect=calculate_order_statistics_side_effect
            ),
...
async def test_get_orderbook_success_with_api_key():
    ...
    with (
        patch(
            "app.core.services.orderbook_service.get_analyze_mode",
            new_callable=AsyncMock,
            return_value=True,
        ) as mock_get_analyze_mode,
        patch(
            "app.core.services.orderbook_service.get_auth_token_broker",
            return_value=("mock_token", "mock_broker"),
        ) as mock_get_auth_token,
        patch("app.core.services.orderbook_service.OrderManager") as MockOrderManager,
    ):
        # OrderManager.get_orderbook is an async method
        mock_order_manager_instance = MockOrderManager.return_value
        mock_order_manager_instance.get_orderbook = AsyncMock(
            return_value=(
                True,
                {
                    "status": "success",
...
async def test_get_orderbook_broker_module_not_found():
    ...
    db.execute.return_value = mock_result
    with patch(
        "app.core.services.orderbook_service.import_broker_module", return_value=None
    ):
        success, response, status_code = await get_orderbook(
            db, auth_token="token", broker="non_existent_broker"
        )
